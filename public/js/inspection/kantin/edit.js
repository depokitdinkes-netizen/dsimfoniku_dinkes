...
$(document).ready(function() {
    ...
    if (window.kantinEditData) {
        const { deleteRouteTemplate } = window.kantinEditData;
        
        $('.btn-open-rm-modal').click(function() {
            let deleteURL = deleteRouteTemplate.substr(0, deleteRouteTemplate.length - 1) + this.id;
...
// Kecamatan and Kelurahan dropdown functionality
$(document).ready(function() {
    // Get data from global window object (passed from Blade template)
    if (window.kantinEditData) {
        const { kecamatan: kecVal, kelurahan: kelVal } = window.kantinEditData;
