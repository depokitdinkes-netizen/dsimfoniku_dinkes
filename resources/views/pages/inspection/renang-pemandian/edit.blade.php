...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Arena Renang atau Pemandian Alam" />

<x-breadcrumb.edit-inspection showRoute="{{ route('renang-pemandian.show', ['renang_pemandian' => $form_data['id']]) }}" />

<form action="{{ route('renang-pemandian.update', ['renang_pemandian' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
...
<div data-window-var="renangPemandianEditData" data-kecamatan="{{ $form_data['kecamatan'] ?? '' }}" data-kelurahan="{{ $form_data['kelurahan'] ?? '' }}" style="display:none;"></div>

<script>
    window.isAuthenticated = {{ auth()->check() ? 'true' : 'false' }};
</script>
<script src="{{ asset('js/getDistrictsAndVillages.js') }}"></script>
<script src="{{ asset('js/inspection/renang-pemandian/edit.js') }}"></script>
