...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Pasar" />

<x-breadcrumb.edit-inspection showRoute="{{ route('pasar.show', ['pasar' => $form_data['id']]) }}" />

<form action="{{ route('pasar.update', ['pasar' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
        ...
        <div class="bg-white rounded-xl p-6 sm:p-8">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    ...
                    <input type="hidden" id="kecamatan-backup" name="kecamatan-backup" value="{{ $form_data['kecamatan'] }}" />
                </div>
                @break
                ...
                <div class="input-group">
                    ...
                    <input type="hidden" id="kelurahan-backup" name="kelurahan-backup" value="{{ $form_data['kelurahan'] }}" />
                </div>
                @break
                ...
                @case('tanggal-penilaian')
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="date" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data[$form_input['name']] ? $form_data[$form_input['name']]->format('Y-m-d') : '' }}" required />
                </div>
                @break
...
<div data-window-var="pasarEditData" data-kecamatan="{{ $form_data['kecamatan'] }}" data-kelurahan="{{ $form_data['kelurahan'] }}" data-id="{{ $form_data['id'] }}" data-duplicate-route="{{ route('pasar.store') }}" data-csrf-token="{{ csrf_token() }}" style="display:none;"></div>
...
<script src="{{ asset('js/inspection/pasar/edit.js') }}"></script>
@endsection
