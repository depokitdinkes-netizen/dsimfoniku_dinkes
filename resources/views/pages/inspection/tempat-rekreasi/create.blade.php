...
@section('content')
<x-section.page-header title="Inspeksi / Penilaian Tempat Rekreasi" />

<x-breadcrumb.create-inspection />

<form action="{{ route('tempat-rekreasi.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('POST')
    <!-- INFORMASI UMUM -->
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    ...
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" 
                           ...
                           title="Masukkan nomor telepon 10-15 digit" 
                           required />
