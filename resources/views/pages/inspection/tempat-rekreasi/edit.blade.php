...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Tempat Rekreasi" />

<x-breadcrumb.edit-inspection showRoute="{{ route('tempat-rekreasi.show', ['tempat_rekreasi' => $form_data['id']]) }}" />

<form action="{{ route('tempat-rekreasi.update', ['tempat_rekreasi' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
    <div class="px-3 pb-3 sm:px-6 sm:pb-6">
        <div class="bg-white rounded-xl p-6 sm:p-8">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    @if($form_data['dokumen_sls'])
                    <div class="mb-2">
                        <span class="text-sm text-gray-600">Dokumen saat ini: </span>
                        <a href="{{ $form_data['dokumen_sls'] }}" target="_blank" class="text-blue-600 hover:underline">
                            {{ $form_data['dokumen_sls'] }}
                        </a>
                    </div>
                    @endif
                    ...
                    <div class="text-sm text-gray-500 mt-1">Masukkan link dokumen SLS (Google Drive, OneDrive, atau penyimpanan cloud lainnya)</div>
                </div>
                @break

                @case('sls_issued_date')
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="date" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data[$form_input['name']] }}" onchange="calculateSlsExpireDate()" />
                </div>
                @break

                @case('sls_expire_date')
                <div class="input-group">
                ...
                <div class="input-group">
                    ...
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" 
                           value="{{ old($form_input['name'], $form_data[$form_input['name']] ?? '') }}"
                           placeholder="Contoh: 08123456789" 
                           required />
                    <div class="text-sm text-gray-500 mt-1">Masukkan nomor telepon 10-15 digit</div>
                </div>
                @break
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ old($form_input['name'], $form_data[$form_input['name']] ?? '') }}" required />
                </div>
                @endswitch
                @endforeach
            </div>
...
<div data-window-var="tempatRekreasiFormData" data-kecamatan="{{ $form_data['kecamatan'] }}" data-kelurahan="{{ $form_data['kelurahan'] }}" style="display:none;"></div>
<script src="{{ asset('js/autosave-form.js') }}"></script>
<script src="{{ asset('js/inspection/tempat-rekreasi/edit.js') }}"></script>
@endsection
