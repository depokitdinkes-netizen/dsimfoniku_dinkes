...
@section('content')
<x-section.page-header title="Inspeksi / Penilaian SAM Sumur Gali dengan Kerekan" />

<x-breadcrumb.create-inspection />

<form action="{{ route('sumur-gali.store') }}" method="POST">
    @csrf
    @method('POST')
    <!-- INFORMASI UMUM -->
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    ...
                    <select id="u006" name="u006" class="select select-bordered" required>
                        ...
                        <option value="Sumur Gali + Mesin Pompa">Sumur Gali + Mesin Pompa</option>
                    </select>
                </div>
                @break
