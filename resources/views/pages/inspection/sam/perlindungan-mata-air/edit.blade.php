...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian SAM Perpipaan Perlindungan Mata Air" />

<x-breadcrumb.edit-inspection showRoute="{{ route('perlindungan-mata-air.show', ['perlindungan_mata_air' => $form_data['id']]) }}" />

<form action="{{ route('perlindungan-mata-air.update', ['perlindungan_mata_air' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
    ...
    <div class="px-3 pb-3 sm:px-6 sm:pb-6 flex flex-wrap lg:flex-nowrap gap-5">
        <div>
            ...
            <div class="bg-white flex-grow pb-4 rounded-xl">
                ...
                <div class="px-3 sm:px-8">
                    <div class="p-4 border rounded mb-3 bg-gray-100">
                        <div class="flex flex-col-reverse sm:flex-row justify-between gap-2 font-medium">
                            <label for="ada-bangunan-penangkap">Apakah ada bangunan penangkap?</label>
                        </div>
                        <hr class="mt-3 mb-2" />
                        <div class="flex gap-5">
                            <div class="form-control">
                                <label class="label cursor-pointer justify-start gap-2">
                                    <input type="radio" name="ada-bangunan-penangkap" class="radio checked:bg-error" value="0" @if($form_data['ada-bangunan-penangkap']=='0' ) checked @endif />
                                    <span class="label-text">Tidak</span>
                                </label>
                            </div>
                            <div class="form-control">
                                <label class="label cursor-pointer justify-start gap-2">
                                    <input type="radio" name="ada-bangunan-penangkap" class="radio checked:bg-primary" value="1" @if($form_data['ada-bangunan-penangkap']=='1' ) checked @endif />
                                    <span class="label-text">Ya</span>
                                </label>
                            </div>
                        </div>
                ...
                @foreach ($data_kpr as $index => $form_input)
                @if($index >= 11)
                <div class="px-3 sm:px-8 extra-question @if($form_data['ada-bangunan-penangkap'] == '0') hidden @endif">
                    <div class="p-4 border rounded mb-3">
                        <div class="flex flex-col-reverse sm:flex-row justify-between gap-2 font-medium">
                            <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                            <span class="badge badge-outline badge-primary">+{{ $form_input['option'][1]['value'] }}</span>
...
<div data-window-var="formData" data-kecamatan="{{ $form_data['kecamatan'] }}" data-kelurahan="{{ $form_data['kelurahan'] }}" style="display:none;"></div>
...
<script src="{{ asset('js/inspection/sam/perlindungan-mata-air/edit.js') }}"></script>
