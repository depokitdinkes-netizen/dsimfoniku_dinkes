...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Puskesmas" />

<x-breadcrumb.edit-inspection showRoute="{{ route('puskesmas.show', ['puskesmas' => $form_data['id']]) }}" />

<form action="{{ route('puskesmas.update', ['puskesmas' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
...
<div data-window-var="puskesmasEditData" data-kecamatan="{{ $form_data['kecamatan'] }}" data-kelurahan="{{ $form_data['kelurahan'] }}" style="display:none;"></div>
...
<script src="{{ asset('js/inspection/puskesmas/edit.js') }}"></script>
