...
@section('content')
<x-section.page-header title="Inspeksi / Penilaian Pasar Internal" />

<x-breadcrumb.create-inspection />

<form action="{{ route('pasar-internal.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('POST')
    ...
    <div class="px-3 pb-3 sm:px-6 sm:pb-6">
        <div class="bg-white p-6 sm:p-8 rounded-xl">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" @if(in_array($form_input['name'], ['subjek', 'alamat', 'kecamatan', 'kelurahan', 'pengelola', 'kontak', 'jumlah_total_pedagang', 'jumlah_total_kios'])) required @endif />
                </div>
                @endswitch
    ...
    <div class="px-3 pb-3 sm:px-6 sm:pb-6 flex flex-wrap lg:flex-nowrap gap-5">
        <div class="bg-white flex-grow pb-4 rounded-xl">
            ...
            <div class="px-6 sm:px-8 pb-3 mt-4">
                <h4 class="font-semibold">{{ $form_input['label'] }}</h4>
            </div>
            @break

            @case('h5')
            <div class="px-6 sm:px-8 pb-3 mt-3">
                <h5 class="font-medium text-sm">{{ $form_input['label'] }}</h5>
            </div>
            @break
