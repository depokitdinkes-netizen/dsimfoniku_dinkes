...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Restoran {{ ucfirst(strtolower($form_data['u009'])) }}" />

<x-breadcrumb.edit-inspection showRoute="{{ route('restoran.show', ['restoran' => $form_data['id']]) }}" />

<form action="{{ route('restoran.update', ['restoran' => $form_data['id']]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
    <div class="px-3 pb-3 sm:px-6 sm:pb-6">
        <div class="bg-white p-6 sm:p-8 rounded-xl">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data[$form_input['name']] }}" @if(!in_array($form_input['name'], ['dokumen_slhs', 'slhs_issued_date', 'slhs_expire_date', 'u006'])) required @endif />
                </div>
                @endswitch
