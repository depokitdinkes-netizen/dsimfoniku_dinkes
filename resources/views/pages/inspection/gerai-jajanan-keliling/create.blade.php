...
@section('content')
<x-section.page-header title="Inspeksi / Penilaian Gerai Pangan Jajanan Keliling Gol {{ $golongan }}" />

<x-breadcrumb.create-inspection />

<form action="{{ route('gerai-jajanan-keliling.store') }}" method="POST">
    @csrf
    @method('POST')
    <!-- INFORMASI UMUM -->
        ...
        <div class="bg-white p-6 sm:p-8 rounded-xl">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    ...
                    <select id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" class="select select-bordered" required>
                        <option value="" disabled selected>Pilih salah satu</option>
                        <option value="Pikulan">Pikulan</option>
                        <option value="Gerobak">Gerobak</option>
                        <option value="Becak/roda tiga">Becak/roda tiga</option>
                ...
                <div class="input-group">
                    ...
                    <div class="join">
                        <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}a" name="{{ $form_input['name'] }}a" class="input input-bordered join-item w-full" required />
                        <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}b" name="{{ $form_input['name'] }}b" class="input input-bordered join-item w-full" required />
                    </div>
                </div>
                @break
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="{{ $form_input['type'] }}" value="{{ $golongan }}" disabled />
                    <input type="hidden" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $golongan }}" required />
                </div>
                @break
...
<script src="{{ asset('js/inspection/gerai-jajanan-keliling/create.js') }}"></script>
