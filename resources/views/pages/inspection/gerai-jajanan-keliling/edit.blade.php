...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Gerai Pangan Jajanan Keliling Gol {{ $form_data['u011'] }}" />

<x-breadcrumb.edit-inspection showRoute="{{ route('gerai-jajanan-keliling.show', ['gerai_jajanan_keliling' => $form_data['id']]) }}" />

<form action="{{ route('gerai-jajanan-keliling.update', ['gerai_jajanan_keliling' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
        ...
        <div class="bg-white p-6 sm:p-8 rounded-xl">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    ...
                    <select id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" class="select select-bordered">
                        <option value="" disabled selected>Pilih salah satu</option>
                        <option value="Pikulan" @if($form_data['u003']=='Pikulan' ) selected @endif>Pikulan</option>
                        <option value="Gerobak" @if($form_data['u003']=='Gerobak' ) selected @endif>Gerobak</option>
                        <option value="Becak/roda tiga" @if($form_data['u003']=='Becak/roda tiga' ) selected @endif>Becak/roda tiga</option>
                        <option value="Sepeda" @if($form_data['u003']=='Sepeda' ) selected @endif>Sepeda</option>
                        <option value="Motor" @if($form_data['u003']=='Motor' ) selected @endif>Motor</option>
                        <option value="Mobil" @if($form_data['u003']=='Mobil' ) selected @endif>Mobil</option>
                    </select>
                </div>
                @break
                ...
                @case('u005')
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }} (Dari - Sampai)</label>
                    <div class="join">
                        <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}a" name="{{ $form_input['name'] }}a" class="input input-bordered join-item w-full" value="{{ $form_data['u005a'] }}" required />
                        <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}b" name="{{ $form_input['name'] }}b" class="input input-bordered join-item w-full" value="{{ $form_data['u005b'] }}" required />
                    </div>
                </div>
                @break
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data['u007'] }}" />
                </div>
                @break
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="{{ $form_input['type'] }}" value="{{ $form_data['u011'] }}" disabled />
                    <input type="hidden" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data['u011'] }}" required />
                </div>
                @break
    ...
    <div class="px-3 pb-3 sm:px-6 sm:pb-6 flex flex-wrap lg:flex-nowrap gap-5">
        ...
        <div class="sticky top-5 h-fit min-w-72 w-full lg:w-fit">
            ...
            <button type="submit" name="action" value="duplicate" class="btn btn-info btn-outline btn-block mt-5">DUPLIKAT PENILAIAN</button>
        </div>
