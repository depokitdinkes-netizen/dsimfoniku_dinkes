...
@section('content')
<x-section.page-header title="Inspeksi / Penilaian Akomodasi Lainnya" />

<x-breadcrumb.create-inspection />

<form action="{{ route('akomodasi-lain.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('POST')
    <!-- INFORMASI UMUM -->
    <div class="px-3 pb-3 sm:px-6 sm:pb-6">
        <div class="bg-white p-6 sm:p-8 rounded-xl">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="date" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" onchange="calculateSlsExpireDate()" />
                </div>
                @break
...
<!-- Load create.js FIRST to set restriction flags -->
<script src="{{ asset('js/inspection/akomodasi-lain/create.js') }}"></script>
<!-- Then load getDistrictsAndVillages.js which will check the flag -->
