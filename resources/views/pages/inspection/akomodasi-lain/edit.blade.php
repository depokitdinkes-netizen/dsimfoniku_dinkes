...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Akomodasi Lainnya" />

<x-breadcrumb.edit-inspection showRoute="{{ route('akomodasi-lain.show', ['akomodasi_lain' => $form_data['id']]) }}" />

<form action="{{ route('akomodasi-lain.update', ['akomodasi_lain' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
...
<div data-window-var="akomodasiLainEditData" data-kecamatan="{{ $form_data['kecamatan'] }}" data-kelurahan="{{ $form_data['kelurahan'] }}" style="display:none;"></div>

<script src="{{ asset('js/getDistrictsAndVillages.js') }}"></script>
<script src="{{ asset('js/inspection/akomodasi-lain/edit.js') }}"></script>
