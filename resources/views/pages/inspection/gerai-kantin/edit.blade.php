...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Gerai Kantin" />
...
<div class="px-3 sm:px-6 py-3">
    <div class="breadcrumbs text-sm">
        <ul>
            ...
            <li><a class="text-blue-500" href="{{ route('kantin.show', ['kantin' => $kantin['id']]) }}">Hasil Inspeksi</a></li>
            <li><a class="text-blue-500" href="{{ route('kantin.edit', ['kantin' => $kantin['id']]) }}">Ubah Inspeksi Kantin</a></li>
            <li>Ubah Inspeksi Gerai</li>
        </ul>
    </div>
</div>

<form action="{{ route('gerai-kantin.update', ['gerai_kantin' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
    <div class="px-3 pb-3 sm:px-6 sm:pb-6">
        <div class="bg-white p-6 sm:p-8 rounded-xl">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                <input type="hidden" name="id-kantin" value="{{ $kantin['id'] }}" />

                @foreach ($informasi_umum_kantin as $form_input)
                ...
                <div class="input-group">
                    <label for="kec-kantin">{{ $form_input['label'] }}</label>
                    <select name="{{ $form_input['name'] }}" id="kec-kantin" class="select select-bordered" disabled>
                ...
                @case('kelurahan')
                <div class="input-group">
                    <label for="kel-kantin">{{ $form_input['label'] }}</label>
                    <select name="{{ $form_input['name'] }}" id="kel-kantin" class="select select-bordered" disabled>
                ...
                <div class="input-group">
                    ...
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $kantin['u006'] }}" disabled />
                </div>
                @break
                ...
                <div class="input-group">
                    ...
                    <select name="{{ $form_input['name'] }}" id="status-operasi" class="select select-bordered" disabled>
                        <option value="" disabled selected>Pilih Status</option>
                        <option @if($kantin['status-operasi']) selected @endif value="1">Masih Beroperasi</option>
                        <option @if(!$kantin['status-operasi']) selected @endif value="0">Tidak Beroperasi</option>
                    </select>
                </div>
                @break
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <div class="join">
                        <input type="{{ $form_input['type'] }}" id="koordinat" name="koordinat" class="join-item w-full" placeholder="-6.324667, 106.891268" required value="{{ $kantin['koordinat'] }}" disabled />
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $kantin[$form_input['name']] }}" disabled />
                </div>
                @endswitch
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                @case('kecamatan')
                <div class="input-group">
                    <label for="kec-gerai">{{ $form_input['label'] }}</label>
                    <select name="{{ $form_input['name'] }}" id="kec-gerai" class="select select-bordered" required>
                        <option value="">Pilih Kecamatan</option>
                    </select>
                </div>
                @break
                ...
                @case('kelurahan')
                <div class="input-group">
                    <label for="kel-gerai">{{ $form_input['label'] }}</label>
                    <select name="{{ $form_input['name'] }}" id="kel-gerai" class="select select-bordered" required>
                        <option value="">Pilih Kelurahan</option>
                    </select>
                </div>
                @break
...
<div data-window-var="geraiKantinEditData" data-kec-kantin="{{ $kantin['kecamatan'] }}" data-kel-kantin="{{ $kantin['kelurahan'] }}" data-kec-gerai="{{ $form_data['kecamatan'] }}" data-kel-gerai="{{ $form_data['kelurahan'] }}" style="display:none;"></div>

<script src="{{ asset('js/getDistrictsAndVillages.js') }}"></script>
<script src="{{ asset('js/autosave-form.js') }}"></script>
<script src="{{ asset('js/inspection/gerai-kantin/edit.js') }}"></script>
@endsection
