...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Akomodasi" />

<x-breadcrumb.edit-inspection showRoute="{{ route('akomodasi.show', ['akomodasi' => $form_data['id']]) }}" />

<form action="{{ route('akomodasi.update', ['akomodasi' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
...
<div data-window-var="akomodasiEditData" data-kecamatan="{{ $form_data['kecamatan'] }}" data-kelurahan="{{ $form_data['kelurahan'] }}" style="display:none;"></div>

<script src="{{ asset('js/getDistrictsAndVillages.js') }}"></script>
<script src="{{ asset('js/inspection/akomodasi/edit.js') }}"></script>
