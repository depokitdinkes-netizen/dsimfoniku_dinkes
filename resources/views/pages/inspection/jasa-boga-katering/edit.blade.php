...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Jasa Boga Katering Gol {{ $form_data['u009'] }}" />

<x-breadcrumb.edit-inspection showRoute="{{ route('jasa-boga-katering.update', ['jasa_boga_katering' => $form_data['id']]) }}" />

<form action="{{ route('jasa-boga-katering.update', ['jasa_boga_katering' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
    <div class="px-3 pb-3 sm:px-6 sm:pb-6">
        <div class="bg-white p-6 sm:p-8 rounded-xl">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    <label for="kel">{{ $form_input['label'] }}</label>
                    <input type="text" value="{{ $form_data['u009'] }}" disabled />
