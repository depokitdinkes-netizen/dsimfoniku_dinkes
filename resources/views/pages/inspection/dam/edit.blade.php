...
@section('content')
<x-section.page-header title="Edit Inspeksi / Penilaian Depot Air Minum" />

<x-breadcrumb.edit-inspection showRoute="{{ route('depot-air-minum.show', ['depot_air_minum' => $form_data['id']]) }}" />
...
<form action="{{ route('depot-air-minum.update', ['depot_air_minum' => $form_data['id']]) }}" method="POST" id="dam-edit-form">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
        ...
        <div class="bg-white p-6 sm:p-8 rounded-xl">
            ...
            <div class="grid grid-flow-row md:grid-cols-2 gap-5">
                ...
                <div class="input-group">
                    ...
                    <select name="{{ $form_input['name'] }}" id="kec" class="select select-bordered @error('kecamatan') select-error @enderror" required>
                ...
                <div class="input-group">
                    ...
                    <select name="{{ $form_input['name'] }}" id="kel" class="select select-bordered @error('kelurahan') select-error @enderror" required>
                ...
                <div class="input-group">
                    ...
                    <select name="{{ $form_input['name'] }}" id="tujuan-ikl" class="select select-bordered @error('tujuan-ikl') select-error @enderror" required>
                        <option value="">Pilih</option>
                        <option value="Program" @if($form_data['tujuan-ikl']=='Program' ) selected @endif>Program</option>
                        <option value="Perizinan SLHS" @if($form_data['tujuan-ikl']=='Perizinan SLHS' ) selected @endif>Perizinan SLHS</option>
                    </select>
                    @error('tujuan-ikl')
                        <span class="text-error text-xs">{{ $message }}</span>
                    @enderror
                ...
                <div class="input-group">
                    ...
                    <select name="{{ $form_input['name'] }}" id="status-operasi" class="select select-bordered @error('status-operasi') select-error @enderror" required>
                ...
                <div class="input-group">
                    ...
                    <input type="url" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data[$form_input['name']] }}" placeholder="https://drive.google.com/... atau link dokumen lainnya" class="input input-bordered @error($form_input['name']) input-error @enderror" />
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="date" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data[$form_input['name']] }}" onchange="calculateSlhsExpireDate()" class="input input-bordered @error($form_input['name']) input-error @enderror" />
                    @error($form_input['name'])
                        <span class="text-error text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="input-group">
                    ...
                    <input type="date" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data[$form_input['name']] }}" readonly class="input input-bordered bg-gray-100 @error($form_input['name']) input-error @enderror" />
                    @error($form_input['name'])
                        <span class="text-error text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <div class="join">
                        <input type="{{ $form_input['type'] }}" id="koordinat" name="koordinat" class="join-item w-full @error('koordinat') input-error @enderror" placeholder="-6.324667, 106.891268" value="{{ $form_data['koordinat'] }}" required />
                ...
                <div class="input-group">
                    <label for="{{ $form_input['name'] }}">{{ $form_input['label'] }}</label>
                    <input type="{{ $form_input['type'] }}" id="{{ $form_input['name'] }}" name="{{ $form_input['name'] }}" value="{{ $form_data[$form_input['name']] }}" class="input input-bordered @error($form_input['name']) input-error @enderror" @if(!in_array($form_input['name'], ['dokumen_slhs', 'slhs_issued_date', 'slhs_expire_date', 'u006'])) required @endif />
                    @error($form_input['name'])
                        <span class="text-error text-xs">{{ $message }}</span>
                    @enderror
                </div>
    ...
    <div class="px-3 pb-3 sm:px-6 sm:pb-6 flex flex-wrap lg:flex-nowrap gap-5">
        ...
        <div class="sticky top-5 h-fit min-w-72 w-full lg:w-fit">
            ...
            <button onclick="document.getElementById('reminder_before_submit').showModal()" type="button" class="btn btn-primary btn-block">SIMPAN PENILAIAN</button>
