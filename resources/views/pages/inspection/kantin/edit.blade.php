...
@section('content')
<x-section.page-header title="Ubah Informasi / Penilaian Kantin" />

<x-breadcrumb.edit-inspection showRoute="{{ route('kantin.show', ['kantin' => $form_data['id']]) }}" />

<form action="{{ route('kantin.update', ['kantin' => $form_data['id']]) }}" method="POST">
    @csrf
    @method('PUT')
    <!-- INFORMASI UMUM -->
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="flex gap-1.5">
                        <a href="{{ route('gerai-kantin.edit', ['gerai_kantin' => $gerai['id']]) }}" class="btn btn-warning">
                            <i class="ri-edit-fill"></i>
                        </a>
                        <button type="button" id="{{ $gerai['id'] }}" class="btn-open-rm-modal btn btn-error btn-outline">
...
<x-modal.remove-inspection destroyRoute="" />

<div data-window-var="kantinEditData" data-delete-route-template="{{ route('gerai-kantin.destroy', ['gerai_kantin' => 0]) }}" data-kecamatan="{{ $form_data['kecamatan'] }}" data-kelurahan="{{ $form_data['kelurahan'] }}" style="display:none;"></div>
...
<script src="{{ asset('js/inspection/kantin/edit.js') }}"></script>
@endsection
