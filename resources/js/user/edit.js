...
function addKelurahanFieldEdit() {
    const container = document.getElementById('kelurahan-fields');
    if (!container) return;
    
    const fieldCount = container.children.length;
    const newField = document.createElement('div');
    newField.className = 'kelurahan-field mb-2';
    newField.innerHTML = `
        <div class="input-group">
            <select name="kelurahan[]" class="form-control kelurahan-select" required>
                <option value="">Pilih Kelurahan</option>
            </select>
            <div class="input-group-append">
                <button type="button" class="btn btn-danger" onclick="removeKelurahanFieldEdit(this)">
...
function addKelurahanFieldEditAdmin() {
    ...
    newField.innerHTML = `
        <div class="input-group">
            <select name="kelurahan[]" class="form-control kelurahan-select-admin" required>
                ${optionsHTML}
            </select>
            <div class="input-group-append">
                <button type="button" class="btn btn-danger" onclick="removeKelurahanFieldEditAdmin(this)">
                    <i class="fas fa-minus"></i>
                </button>
            </div>
        </div>
...
function updateKelurahanOptionsEdit() {
    const selectedKecamatan = document.getElementById('selected-kecamatan')?.value;
    if (!selectedKecamatan || !window.kelurahanData) return;
    
    const kelurahanSelects = document.querySelectorAll('.kelurahan-select');
    const selectedValues = Array.from(kelurahanSelects).map(select => select.value);
    
    kelurahanSelects.forEach((select, index) => {
        const currentValue = select.value;
        select.innerHTML = '<option value="">Pilih Kelurahan</option>';
        
        if (window.kelurahanData[selectedKecamatan]) {
            window.kelurahanData[selectedKecamatan].forEach(kelurahan => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const { userKelurahan, user, authUserRole, kopSuratPreviewRoute } = window.userEditData || {};
...
function handleKecamatanChange(event) {
    ...
    document.getElementById('selected-kecamatan').value = selectedKecamatan;
    setTimeout(updateKelurahanOptionsEdit, 1500);
}

function handleKecamatanChangeAdmin(event) {
    const selectedKecamatan = event.target.value;
    document.getElementById('selected-kecamatan-admin').value = selectedKecamatan;
