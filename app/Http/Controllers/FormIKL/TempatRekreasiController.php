...
class TempatRekreasiController extends Controller
{
    ...
    protected function formPenilaian()
    {
        return [
            ...
            Form::checkbox('Memiliki ruangan yang berfungsi untuk tempat kerja, ruang tempat rekreasi, ruang tunggu, dan ruang untuk penyimpanan perlengkapan pendukung yang memadai', 't002', 2),
            Form::checkbox('Memiliki perlengkapan pendukung untuk usaha tempat rekreasi (furnitur, meja, dll) yang memadai', 't003', 2),
            Form::checkbox('Memiliki papan nama tempat usaha tempat rekreasi', 't004', 2),
            Form::checkbox('Memiliki sistem pelayanan usaha tempat rekreasi yang terstandar', 't005', 2),

            Form::h(2, 'Peralatan (tempat rekreasi selain usaha SPA)', 'peralatan-tempat-rekreasi-selain-usaha-spa'),
            ...
            Form::h(2, 'Peralatan dan Perlengkapan Untuk Usaha SPA', 'peralatan-dan-perlengkapan-untuk-usaha-spa'),

            Form::checkbox('Peralatan dan perlengkapan mengacu pada Peraturan Menteri Kesehatan tentang Pelayanan Kesehatan SPA', 'p006', 10),
            ...
            Form::checkbox('Petugas kebersihan memiliki sertifikat pelatihan petugas kebersihan (cleaning service)', 'pk002', 5),
            ...
            Form::h(2, 'Ketersediaan sarana dan bangunan untuk pengunjung selain usaha SPA', 'ketersediaan-sarana-dan-bangunan-untuk-pengunjung-selain-usaha-spa'),
            ...
            Form::checkbox('Tersedia ruang tempat rekreasi dan perlengkapannya dalam keadaan bersih', 'sb005', 2),
            ...
            Form::h(2, 'Ketersediaan sarana dan bangunan untuk usaha SPA', 'ketersediaan-sarana-dan-bangunan-untuk-usaha-spa'),

            Form::checkbox('Sarana dan bangunan mengacu pada Peraturan Menteri Kesehatan tentang Pelayanan Kesehatan SPA', 'sb010', 19),
            ...
            Form::checkbox('Pengujian contoh air minum wajib dilakukan oleh pihak tempat rekreasi di Laboratorium Pemeriksaan Kualitas Air yang ditunjuk oleh Pemerintah Kabupaten/kota atau yang terakreditasi', 'a002', 2),
            ...
            Form::checkbox('Pengujian contoh udara wajib dilakukan oleh pihak usaha tempat rekreasi di Laboratorium Pemeriksaan Kualitas udara yang ditunjuk oleh Pemerintah Kabupaten/kota atau yang terakreditasi', 'uk004', 2),
            ...
            Form::h(2, 'Penilaian sendiri/mandiri (Self Assessment)', 'penilaian-mandiri-self-assessment'),

            Form::checkbox('Setiap pengelola/penanggung jawab wajib melakukan pengawasan terhadap pemenuhan standar baku mutu kesehatan lingkungan dan persyaratan kesehatan tempat rekreasi secara terus menerus dan dilaporkan satu kali dalam setahun dalam bentuk penilaian sendiri/mandiri (self assessment)', 's001', 3)];
    }
    ...
    public function index(Request $request)
    {
        switch ($request->export) {
            case 'pdf':
                $item = TempatRekreasi::find($request->id);
                ...
                return Pdf::loadView('pdf', [
                    ...
                    'pemeriksa' => $item['nama-pemeriksa']])->download('BAIKL_TEMPAT_REKREASI_' . str_pad($item['id'], 5, '0', STR_PAD_LEFT) . '.pdf');
            case 'excel':
                return Excel::download(new class implements FromCollection, WithHeadings {
                    public function collection()
                    {
                        return TempatRekreasi::withTrashed()->get()->map(function ($item) {
                    ...
                    public function headings(): array
                    {
                        return [
                            ...
                            'Memiliki ruangan yang berfungsi untuk tempat kerja, ruang tempat rekreasi, ruang tunggu, dan ruang untuk penyimpanan perlengkapan pendukung yang memadai',
                            'Memiliki perlengkapan pendukung untuk usaha tempat rekreasi (furnitur, meja, dll) yang memadai',
                            'Memiliki papan nama tempat usaha tempat rekreasi',
                            'Memiliki sistem pelayanan usaha tempat rekreasi yang terstandar',
                            ...
                            'Peralatan dan perlengkapan mengacu pada Peraturan Menteri Kesehatan tentang Pelayanan Kesehatan SPA',
                            'Penjamah makanan memiliki sertifikat pelatihan higiene sanitasi makanan',
                            'Petugas kebersihan memiliki sertifikat pelatihan petugas kebersihan (cleaning service)',
                            ...
                            'Tersedia ruang tempat rekreasi dan perlengkapannya dalam keadaan bersih',
                            ...
                            'Sarana dan bangunan mengacu pada Peraturan Menteri Kesehatan tentang Pelayanan Kesehatan SPA',
                            ...
                            'Pengujian contoh air minum wajib dilakukan oleh pihak tempat rekreasi di Laboratorium Pemeriksaan Kualitas Air yang ditunjuk oleh Pemerintah Kabupaten/kota atau yang terakreditasi',
                            ...
                            'Pengujian contoh udara wajib dilakukan oleh pihak usaha tempat rekreasi di Laboratorium Pemeriksaan Kualitas udara yang ditunjuk oleh Pemerintah Kabupaten/kota atau yang terakreditasi',
                            ...
                            'Setiap pengelola/penanggung jawab wajib melakukan pengawasan terhadap pemenuhan standar baku mutu kesehatan lingkungan dan persyaratan kesehatan tempat rekreasi secara terus menerus dan dilaporkan satu kali dalam setahun dalam bentuk penilaian sendiri/mandiri (self assessment)',
                            'Link Upload Dokumen Sertifikat Laik Sehat (SLS)',
                            'Tanggal Terbit Dokumen SLS',
                            'Tanggal Berakhir Dokumen SLS'];
                    }
                }, 'REPORT_TEMPAT_REKREASI_' . Carbon::now()->format('Ymd') . '.xlsx');
            default:
                abort(404);
        }
    }
    ...
    {
        return view('pages.inspection.tempat-rekreasi.create', [
    ...
    public function store(Request $request)
    {
        try {
            ...
            $validatedData = $request->validate([
                ...
                'status-operasi' => 'required|in:1,0',
                ...
                'status-operasi.in' => 'Status operasi harus dipilih (Beroperasi atau Tidak Beroperasi).',
                'kontak.required' => 'Kontak pengelola wajib diisi.',
                'kontak.max' => 'Kontak pengelola maksimal 255 karakter.',
            ...
            $data['skor'] = (int) ((int) (array_reduce($this->formPenilaianName(), fn($carry, $column) => $carry + $request->input($column), 0) / 100 * 100) / 129 * 100);

            $insert = TempatRekreasi::create($data);
            ...
            return redirect(route('tempat-rekreasi.show', ['tempat_rekreasi' => $insert->id]))
                ->with('success', 'Penilaian/inspeksi Tempat Rekreasi berhasil dibuat.');
    ...
    public function show(TempatRekreasi $tempatRekreasi)
    {
        return view('pages.inspection.show', [
            'page_name' => 'history',
            'form_data' => $tempatRekreasi,
            'general_info' => $this->informasiUmum(),
            'inspection_name' => 'Tempat Rekreasi',
            'edit_route' => route('tempat-rekreasi.edit', ['tempat_rekreasi' => $tempatRekreasi['id']]),
            'destroy_route' => route('tempat-rekreasi.destroy', ['tempat_rekreasi' => $tempatRekreasi['id']]),
            'export_route' => route(
                'tempat-rekreasi.index',
                [
                    'export' => 'pdf',
                    'id' => $tempatRekreasi['id']],
            )]);
    }
    ...
    public function update(Request $request, TempatRekreasi $tempatRekreasi)
    {
        try {
            Log::info('Tempat Rekreasi update process started', [
                'user_id' => Auth::id(),
                'record_id' => $tempatRekreasi->id
            ]);
            ...
            if ($data['action'] == 'duplicate') {
                ...
                $duplicate = TempatRekreasi::where('subjek', $data['subjek'])
                    ->where('alamat', $data['alamat'])
                    ->where('id', '!=', $tempatRekreasi->id)
                    ->first();

                if ($duplicate) {
                    Log::warning('Duplicate Tempat Rekreasi found during duplication', [
                        'user_id' => Auth::id(),
                        'original_id' => $tempatRekreasi->id,
                ...
                if (!$insert) {
                    Log::error('Failed to duplicate Tempat Rekreasi record', [
                        'user_id' => Auth::id(),
                        'original_id' => $tempatRekreasi->id,
                        'data' => $validatedData
                    ]);
                    return redirect(route('inspection'))->with('error', 'Penilaian/inspeksi Tempat Rekreasi gagal diduplikasi, silahkan coba lagi.');
                }

                Log::info('Tempat Rekreasi record duplicated successfully', [
                    'user_id' => Auth::id(),
                    'original_id' => $tempatRekreasi->id,
                    'new_id' => $insert->id,
                    'subjek' => $insert->subjek
                ...
                return redirect(route('tempat-rekreasi.show', ['tempat_rekreasi' => $insert->id]))->with('success', 'Penilaian/inspeksi Tempat Rekreasi berhasil diduplikasi.');
            }
            ...
            if ($duplicate) {
                Log::warning('Duplicate Tempat Rekreasi found during update', [
                    'user_id' => Auth::id(),
                    'record_id' => $tempatRekreasi->id,
                    'duplicate_id' => $duplicate->id,
                    'subjek' => $data['subjek'],
            ...
            $update = $tempatRekreasi->update($data);

            if (!$update) {
                Log::error('Failed to update Tempat Rekreasi record', [
                    'user_id' => Auth::id(),
                    'record_id' => $tempatRekreasi->id,
                    'data' => $validatedData
                ]);
                return redirect()->back()
                    ->withInput()
                    ->with('error', 'Form informasi dan penilaian Tempat Rekreasi gagal diubah. Silahkan coba lagi.');
            }

            Log::info('Tempat Rekreasi record updated successfully', [
                'user_id' => Auth::id(),
                'record_id' => $tempatRekreasi->id,
                'subjek' => $tempatRekreasi->subjek
            ]);
            ...
            return redirect(route('tempat-rekreasi.show', ['tempat_rekreasi' => $tempatRekreasi['id']]))->with('success', 'Form informasi dan penilaian Tempat Rekreasi berhasil diubah.');

        } catch (ValidationException $e) {
            Log::warning('Tempat Rekreasi update validation failed', [
                'user_id' => Auth::id(),
                'record_id' => $tempatRekreasi->id,
                'errors' => $e->errors(),
                'input' => $request->except(['_token'])
            ...
            Log::error('Unexpected error during Tempat Rekreasi update', [
                'user_id' => Auth::id(),
                'record_id' => $tempatRekreasi->id,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
    ...
    public function destroy(String $id)
    {
        ...
        if ($tempatRekreasi['deleted_at']) {
            $tempatRekreasi->update([
                'deleted_at' => null]);
            return redirect(route('archived'))->with('success', 'form inspeksi berhasil dipulihkan');
        }

        $destroy = $tempatRekreasi->destroy($tempatRekreasi['id']);

        if (!$destroy) {
            return redirect(route('tempat-rekreasi.show', ['tempat_rekreasi' => $tempatRekreasi['id']]))->with('error', 'form informasi dan penilaian Tempat Rekreasi gagal dihapus');
        }

        return redirect(route('history'))->with('success', 'form informasi dan penilaian Tempat Rekreasi berhasil dihapus');
    }
    }