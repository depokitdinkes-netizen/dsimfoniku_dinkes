...
return new class extends Migration {
    ...
    public function up(): void {
        Schema::create('akomodasi_lain', function (Blueprint $table) {
            $table->id();
            $table->string('subjek');
            ...
            $table->enum('pk001', [0, 4]);
            $table->enum('pk002', [0, 4]);
            $table->enum('pk003', [0, 4]);
            ...
            $table->enum('uk002', [0, 3]);
            $table->enum('uk003', [0, 3]);
            $table->enum('uk004', [0, 2]);
            $table->enum('uk005', [0, 2]);

            $table->enum('k001', [0, 12]);

            $table->enum('pl001', [0, 4]);
            $table->enum('pl002', [0, 2]);
    ...
    public function down(): void {
        Schema::dropIfExists('akomodasi_lain');
    }
};
