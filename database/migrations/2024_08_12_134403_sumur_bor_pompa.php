...
return new class extends Migration {
    ...
    public function up(): void {
        Schema::create('sumur_bor_pompa', function (Blueprint $table) {
            $table->id();
            $table->string('subjek');
            ...
            $table->string('u001');
            $table->enum('u006', ['0-15', '15-30', '>30']);
            $table->enum('u007', ['HUJAN_LEBAT', 'HUJAN', 'PANAS']);
            $table->string('u008');
            $table->enum('u009', [0, 1, -1]);
            $table->string('u009a')->nullable();
            $table->enum('u010', [0, 1]);
            $table->enum('u010a', ['BANJIR', 'KEMARAU', 'LISTRIK_PADAM', 'POMPA_RUSAK'])->nullable();
    ...
    public function down(): void {
        Schema::dropIfExists('sumur_bor_pompa');
    }
};
